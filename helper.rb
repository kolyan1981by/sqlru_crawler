...
class Helper
  ...
  def self.download_page(url, use_tor = true)
    headers = { 'User-Agent' => 'Mozilla/5.0 (Windows NT 6.3; rv:36.0) Gecko/20100101 Firefox/36.0'}
    if use_tor
      t=Tor.new
      t.proxy_mechanize do
        a = Mechanize.new
        page =  a.get(url,headers)
        return Nokogiri::HTML(page.body, nil, ENCODING)
      end
    else
      uri = URI.parse(url)
      page = Nokogiri::HTML(open(url,headers), nil, ENCODING)
      return page
    end
  end
